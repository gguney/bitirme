...
<div class="menu">
<INPUT name="Button2"  TYPE="button" onclick="location.href='analyzeCategory.php';" VALUE="Back" class="formButton">
</div>
...
$strSQL = "select DISTINCT cat.PY from bil496.catanalysis as cat,bil496.categories as ca where cat.CID = ca.CID and PY>=".$startPY." and ".$endPY.">=PY and CName='".$field0."' OR CName='".$field1."' OR CName='".$field2."' OR CName='".$field3."' OR CName='".$field4."' ORDER BY ".$order." ".$orderType.";";
...
echo "<td class='analyze'><table>";
	$total =0;
$lastYear = 0;
$strSQL = "select cat.CID,CName,cat.PY,cat.TCount from bil496.catanalysis as cat,bil496.categories as ca where cat.CID = ca.CID and PY>=".$startPY." and ".$endPY.">=PY and CName='".$fieldName."' ORDER BY ".$order." ".$orderType.";";
 ...
 while($row = mysql_fetch_array($result))//SQL Printing 
	{     
		...
		if ($headerDone == 0 )
		{
						...
						echo "<tr class='analyze'><td><a class='link' href='profileCategory.php?".$row[0]."*0+50*AID*ASC'><label class='midLabel2' title='".$row[1]."'>".substr($row[1],0,4)."</label></a></td></tr>";			
