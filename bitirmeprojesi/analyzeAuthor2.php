...
<div class="menu">
<INPUT name="Button2"  TYPE="button" onclick="location.href='analyzeAuthor.php';" VALUE="Back" class="formButton">
</div>
...
$strSQL = "select DISTINCT aua.PY from bil496.auanalysis as aua,bil496.authors as au where aua.AUID = au.AUID and PY>=".$startPY." and ".$endPY.">=PY and AUName='".$field0."' OR AUName='".$field1."' OR AUName='".$field2."' OR AUName='".$field3."' OR AUName='".$field4."' ORDER BY ".$order." ".$orderType.";";
...
echo "<td class='analyze'><table>";
	$total =0;
$lastYear = 0;
$strSQL = "select aua.AUID,AUName,aua.PY,aua.TCount from bil496.auanalysis as aua,bil496.authors as au where aua.AUID = au.AUID and PY>=".$startPY." and ".$endPY.">=PY and AUName='".$fieldName."' ORDER BY ".$order." ".$orderType.";";
 ...
 while($row = mysql_fetch_array($result))//SQL Printing 
	{     
		...
		if ($headerDone == 0 )
		{
						...
						echo "<tr class='analyze'><td><a class='link' href='searchAuthor2.php?".$row[1]."*0+50*TCount*DESC'><label class='midLabel2' title='".$row[1]."'>".substr($row[1],0,4)."</label></a></td></tr>";			
