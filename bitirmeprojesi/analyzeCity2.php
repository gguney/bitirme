...
<div class="menu">
<INPUT name="Button2"  TYPE="button" onclick="location.href='analyzeCity.php';" VALUE="Back" class="formButton">
</div>
...
$strSQL = "select DISTINCT cia.PY from bil496.cianalysis as cia,bil496.cities as ci where cia.CiID = ci.CiID and PY>=".$startPY." and ".$endPY.">=PY and CiName='".$field0."' OR CiName='".$field1."' OR CiName='".$field2."' OR CiName='".$field3."' OR CiName='".$field4."' ORDER BY ".$order." ".$orderType.";";
...
echo "<td class='analyze'><table>";
	$total =0;
$lastYear = 0;
$strSQL = "select cia.CiID,CiName,cia.PY,cia.TCount from bil496.cianalysis as cia,bil496.cities as ci where cia.CiID = ci.CiID and PY>=".$startPY." and ".$endPY.">=PY and CiName='".$fieldName."' ORDER BY ".$order." ".$orderType.";";
 ...
 while($row = mysql_fetch_array($result))//SQL Printing 
	{     
		...
		if ($headerDone == 0 )
		{
						...
						echo "<tr class='analyze'><td><a class='link' href='searchCity2.php?".$row[1]."*0+50*IName*ASC'><label class='midLabel2' title='".$row[1]."'>".substr($row[1],0,4)."</label></a></td></tr>";			
