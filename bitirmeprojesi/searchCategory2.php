...
<INPUT name="Button2"  TYPE="button" onclick="location.href='searchCategory.php';" VALUE="Search Category" class="formButton2">
</div>
<?php 
...
<?php
	...
	$strSQL = "Select CID,CName As Categories,TCount as Total_Count from bil496.categories where CName LIKE '".$field."%' ORDER BY ".$order." ".$orderType." limit ".$offset.", ".$limit.";";					
		...
		if($columnsDone == 0) //Column Names Printing 
		{
			...
			echo "<tr>";
				...
				{		
						...
						{
						echo "<td><span id='round' class='description'><a class='link' href='profileCategory.php?".$row[$i-1]."*0+50*AID*ASC'>".$row[$i]."</a></span></td>";			
...
function decrease()
{
	...
	var orderType = "<?php echo $orderType; ?>";
 	offset = offset - limit;
		...
		window.location.href="searchCategory2.php"+"?"+field+"*"+offset+"+"+limit+"*"+order+"*"+orderType;
