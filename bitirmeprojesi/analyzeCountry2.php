...
<div class="menu">
<INPUT name="Button2"  TYPE="button" onclick="location.href='analyzeCountry.php';" VALUE="Back" class="formButton">
</div>
...
$strSQL = "select DISTINCT coa.PY from bil496.coanalysis as coa,bil496.countries as co where coa.CoID = co.CoID and PY>=".$startPY." and ".$endPY.">=PY and CoName='".$field0."' OR CoName='".$field1."' OR CoName='".$field2."' OR CoName='".$field3."' OR CoName='".$field4."' ORDER BY ".$order." ".$orderType.";";
...
echo "<td class='analyze'><table>";
	$total =0;
$lastYear = 0;
$strSQL = "select coa.CoID,CoName,coa.PY,coa.TCount from bil496.coanalysis as coa,bil496.countries as co where coa.CoID = co.CoID and PY>=".$startPY." and ".$endPY.">=PY and CoName='".$fieldName."' ORDER BY ".$order." ".$orderType.";";
 ...
 while($row = mysql_fetch_array($result))//SQL Printing 
	{     
		...
		if ($headerDone == 0 )
		{
						...
						echo "<tr class='analyze'><td><a class='link' href='searchCountry2.php?".$row[1]."*0+50*IName*ASC'><label class='midLabel2' title='".$row[1]."'>".substr($row[1],0,4)."</label></a></td></tr>";			
