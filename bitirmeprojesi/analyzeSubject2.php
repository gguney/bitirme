...
<div class="menu">
<INPUT name="Button2"  TYPE="button" onclick="location.href='analyzeSubject.php';" VALUE="Back" class="formButton">
</div>
...
$strSQL = "select DISTINCT suba.PY from bil496.subanalysis as suba,bil496.subjects as sub where suba.SID = sub.SID and PY>=".$startPY." and ".$endPY.">=PY and SName='".$field0."' OR SName='".$field1."' OR SName='".$field2."' OR SName='".$field3."' OR SName='".$field4."' ORDER BY ".$order." ".$orderType.";";
...
echo "<td class='analyze'><table>";
	$total =0;
$lastYear = 0;
$strSQL = "select suba.SID,SName,suba.PY,suba.TCount from bil496.subanalysis as suba,bil496.subjects as sub where suba.SID = sub.SID and PY>=".$startPY." and ".$endPY.">=PY and SName='".$fieldName."' ORDER BY ".$order." ".$orderType.";";
 ...
 while($row = mysql_fetch_array($result))//SQL Printing 
	{     
		...
		if ($headerDone == 0 )
		{
						...
						echo "<tr class='analyze'><td><a class='link' href='profileSubject.php?".$row[0]."*0+50*RID*ASC'><label class='midLabel2' title='".$row[1]."'>".substr($row[1],0,4)."</label></a></td></tr>";			
